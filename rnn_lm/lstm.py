...
# vocab_size - Number of unique characters used in the training set.
...
def main(argv):
	...
	shakespeare = DataHandler('traindata.txt', 'testdata.txt', 'validationdata.txt')
	...
	if load_checkpoint and os.path.isfile(os.path.join(cud, checkpoint_file)):
		...
		while step < max_steps and num_plateaus < max_plateaus:
			...
			train_x, train_y = shakespeare.get_random_batch(num_timesteps, batch_size, 'train')
			...
			if step % int(sample_step_percentage*float(max_steps)) == 0:
				...
				training_output = network.run(shakespeare.string_to_tensor('the '), vtoc, ctov, num_steps=500)
